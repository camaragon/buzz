...
**Partial rollout:** pre-doorbell ACP harnesses have no workflow-owner attribution, so their ordinary author gate drops relay-signed workflow messages. A doorbell-aware ACP likewise drops a relay delivery without the mandatory provenance. Delegated workflow prompts are therefore ignored until relay and ACP both support this protocol, while ordinary messages are unaffected.
