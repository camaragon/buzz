...
android {
    ...
    buildTypes {
        debug {
            ...
            val resolvedDebugAppName =
                debugAppName ?: worktreeAppName ?: worktreeLabel?.let { "Buzz ($it)" }
            if (resolvedDebugAppName != null) {
                resValue("string", "app_name", resolvedDebugAppName)
            }
        }
        release {
            if (hasUploadSigning) {
